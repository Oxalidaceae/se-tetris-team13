...
public class Board {
    ...
    /**
     * 아이템 미노를 위한 특별한 placePiece 메서드
     * 지정된 블록 위치에는 100번대 값(아이템 블록)을 배치합니다.
     */
    public void placeItemPiece(int[][] shape, int px, int py, int value, int itemBlockIndex, String itemType) {
        synchronized (lock) {
            int blockCount = 0;
            for (int r = 0; r < shape.length; r++) {
                for (int c = 0; c < shape[r].length; c++) {
