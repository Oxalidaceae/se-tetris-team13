    ...
    void testTitle() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertEquals(
                            "Key Settings", title.getText(), "Title text should be 'Key Settings'");
                    assertTrue(
    ...
    void testMoveLeftButton() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertTrue(
                            leftBtn.getText().startsWith("Move Left: "),
    ...
    void testMoveRightButton() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertTrue(
                            rightBtn.getText().startsWith("Move Right: "),
                            "Right button should start with 'Move Right: '");
                    assertNotNull(
                            rightBtn.getOnAction(), "Right button should have action handler");
                });
    ...
    void testMoveDownButton() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertTrue(
                            downBtn.getText().startsWith("Move Down: "),
    ...
    void testRotateButton() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertTrue(
                            rotateBtn.getText().startsWith("Rotate: "),
                            "Rotate button should start with 'Rotate: '");
                    assertNotNull(
                            rotateBtn.getOnAction(), "Rotate button should have action handler");
                });
    ...
    void testDropButton() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertTrue(
                            dropBtn.getText().startsWith("Drop: "),
    ...
    void testPauseButton() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertTrue(
                            pauseBtn.getText().startsWith("Pause: "),
                            "Pause button should start with 'Pause: '");
                    assertNotNull(
                            pauseBtn.getOnAction(), "Pause button should have action handler");
                });
    ...
    void testBackButton() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertTrue(
                            layout.getChildren().get(8) instanceof Button,
        ...
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertEquals(
                            8,
                            buttonCount,
                            "Should have exactly 8 buttons (7 key settings + back)");
                });
    ...
    void testDefaultKeySettingsDisplay() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertTrue(
                            leftBtn.getText()
                                    .contains(KeyCode.valueOf(settings.getKeyLeft()).getName()),
                            "Left button should display current key setting");
                    assertTrue(
                            rightBtn.getText()
                                    .contains(KeyCode.valueOf(settings.getKeyRight()).getName()),
                            "Right button should display current key setting");
                    assertTrue(
                            downBtn.getText()
                                    .contains(KeyCode.valueOf(settings.getKeyDown()).getName()),
                            "Down button should display current key setting");
                    assertTrue(
                            rotateBtn
                                    .getText()
                                    .contains(KeyCode.valueOf(settings.getKeyRotate()).getName()),
                            "Rotate button should display current key setting");
                    assertTrue(
                            dropBtn.getText()
                                    .contains(KeyCode.valueOf(settings.getKeyDrop()).getName()),
                            "Drop button should display current key setting");
                    assertTrue(
                            pauseBtn.getText()
                                    .contains(KeyCode.valueOf(settings.getPause()).getName()),
    ...
    void testKeyEventFilterRegistered() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertDoesNotThrow(
                            () -> scene.getEventDispatcher(), "Scene should have event dispatcher");
                });
    ...
    void testUndefinedKeyIsRestricted() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertEquals(
                            "Undefined", key.getName(), "UNDEFINED key name should be 'Undefined'");
                });
        ...
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertEquals(
                            "Print Screen",
                            key.getName(),
                            "PRINTSCREEN key name should be 'Print Screen'");
                });
    ...
    void testNumLockKeyIsRestricted() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertEquals(
                            "Num Lock", key.getName(), "NUM_LOCK key name should be 'Num Lock'");
                });
        ...
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    for (KeyCode common : commonKeys) {
                        for (KeyCode restricted : restrictedKeys) {
                            assertNotEquals(
                                    restricted, common, common.getName() + " should be allowed");
                        }
                    }
                });
                    ...
                    for (KeyCode numpad : numpadKeys) {
                        for (KeyCode restricted : restrictedKeys) {
                            assertNotEquals(
                                    ...
                                    numpad,
                                    numpad.getName()
                                            + " should be allowed (not in restricted list)");
                        }
                    }
                });
