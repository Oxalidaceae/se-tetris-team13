...
public class GameOverSceneTest {
    ...
    private static final ScoreBoard.ScoreEntry.Mode TEST_DIFFICULTY = ScoreBoard.ScoreEntry.Mode.NORMAL;
    ...
    void setUp() {
        ...
        javafx.application.Platform.runLater(() -> {
            ...
            gameOverScene = new GameOverScene(sceneManager, settings, TEST_SCORE, TEST_DIFFICULTY);
        });
    ...
    void testGameOverTitle() {
        javafx.application.Platform.runLater(() -> {
            ...
            assertEquals("Game Over!", title.getText(), "Title text should be 'Game Over!'");
        });
    ...
    void testScoreDisplay() {
        javafx.application.Platform.runLater(() -> {
            ...
            assertTrue(layout.getChildren().get(1) instanceof Label, "Second child should be Label");
            Label scoreLabel = (Label) layout.getChildren().get(1);

            assertEquals("Your Score: " + TEST_SCORE, scoreLabel.getText(), "Score label should display the correct score");
        });
    ...
    void testDifficultyDisplay() {
        javafx.application.Platform.runLater(() -> {
            ...
            assertTrue(layout.getChildren().get(2) instanceof Label, "Third child should be Label");
            Label difficultyLabel = (Label) layout.getChildren().get(2);

            assertEquals("Difficulty: " + TEST_DIFFICULTY.name(), difficultyLabel.getText(),
    ...
    void testNameInputField() {
        javafx.application.Platform.runLater(() -> {
            ...
            assertTrue(layout.getChildren().get(3) instanceof TextField, "Fourth child should be TextField");
            TextField nameField = (TextField) layout.getChildren().get(3);

            assertEquals("Enter your name", nameField.getPromptText(), "Name field should have correct prompt text");
            assertTrue(nameField.getText().isEmpty(), "Name field should be initially empty");
        });
    ...
    void testStatusLabel() {
        javafx.application.Platform.runLater(() -> {
            ...
            assertTrue(layout.getChildren().get(4) instanceof Label, "Fifth child should be status Label");
            Label statusLabel = (Label) layout.getChildren().get(4);

            assertTrue(statusLabel.getText().isEmpty() || statusLabel.getText() != null, "Status label should exist");
        });
    ...
    void testSaveScoreButton() {
        javafx.application.Platform.runLater(() -> {
            ...
            assertEquals("Save Score", saveBtn.getText(), "Save button text should be 'Save Score'");
    ...
    void testBackToMenuButton() {
        javafx.application.Platform.runLater(() -> {
            ...
            assertEquals("Back to Menu", backBtn.getText(), "Back button text should be 'Back to Menu'");
    ...
    void testDifferentScores() {
        javafx.application.Platform.runLater(() -> {
            ...
            for (int score : testScores) {
                GameOverScene scene = new GameOverScene(sceneManager, settings, score, TEST_DIFFICULTY);
                Scene s = scene.getScene();
                VBox layout = (VBox) s.getRoot();
                Label scoreLabel = (Label) layout.getChildren().get(1);

                assertEquals("Your Score: " + score, scoreLabel.getText(), "Score should be displayed correctly for score: " + score);
            }
        });
    ...
    void testDifferentDifficulties() {
        javafx.application.Platform.runLater(() -> {
            ...
            for (ScoreBoard.ScoreEntry.Mode difficulty : difficulties) {
                GameOverScene scene = new GameOverScene(sceneManager, settings, TEST_SCORE, difficulty);
                Scene s = scene.getScene();
                VBox layout = (VBox) s.getRoot();
                Label difficultyLabel = (Label) layout.getChildren().get(2);

                assertEquals("Difficulty: " + difficulty.name(), difficultyLabel.getText(),
                        "Difficulty should be displayed correctly for: " + difficulty.name());
            }
        });
    ...
    void testEmptyNameSaveAttempt() {
        javafx.application.Platform.runLater(() -> {
            ...
            assertEquals("Please enter your name before saving!", statusLabel.getText(),
