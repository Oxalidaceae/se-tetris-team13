...
public class SettingSceneTest {
    ...
    void testButtonOrder() {
        javafx.application.Platform.runLater(() -> {
            ...
            // Title 확인
            Label title = (Label) layout.getChildren().get(0);
            assertEquals(expectedTexts[0], title.getText(), "Title should be at position 0");

            // Small, Medium, Large 버튼 확인
            for (int i = 1; i <= 3; i++) {
                Button btn = (Button) layout.getChildren().get(i);
                assertEquals(expectedTexts[i], btn.getText(), "Button at position " + i + " should have correct text");
            }

            // Key Settings 버튼 확인
            Button keyBtn = (Button) layout.getChildren().get(4);
            assertEquals(expectedTexts[4], keyBtn.getText(), "Key Settings button should be at position 4");

            // Reset, Restore, Back 버튼 확인
            for (int i = 6; i <= 8; i++) {
                Button btn = (Button) layout.getChildren().get(i);
                assertEquals(expectedTexts[i - 1], btn.getText(), "Button at position " + i + " should have correct text");
            }
        });
