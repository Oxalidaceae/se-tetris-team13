...
public class GameSceneGhostTest {
    ...
    void testGhostBlockRenderedWhenDifferentFromCurrent() {
        javafx.application.Platform.runLater(() -> {
            ...
            if (ghostY != -1 && ghostY != currentY) {
                ...
                assertNotNull(scene.getRoot(), "Scene root should exist after ghost rendering");
            }
        });
    ...
    void testGhostBlockStaysWithinBounds() {
        javafx.application.Platform.runLater(() -> {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                int ghostY = gameEngine.getGhostY();
                if (ghostY != -1) {
                    assertTrue(ghostY >= 0 && ghostY < BOARD_HEIGHT,
    ...
    void testGhostBlockUpdatesAfterRotation() {
        javafx.application.Platform.runLater(() -> {
            ...
            if (newGhostY != -1) {
                assertTrue(newGhostY >= 0 && newGhostY < BOARD_HEIGHT,
                    "Ghost Y after rotation should be valid: " + newGhostY);
                
                // 회전된 블록이 고스트 위치에 배치 가능한지 확인
                Tetromino current = gameEngine.getCurrent();
                if (current != null) {
                    assertTrue(board.fits(current.getShape(), gameEngine.getPieceX(), newGhostY),
    ...
    void testGhostBlockUpdatesAfterMovement() {
        javafx.application.Platform.runLater(() -> {
            ...
            // 고스트 위치들이 유효한지 확인
            if (ghostYLeft != -1) {
                assertTrue(ghostYLeft >= 0 && ghostYLeft < BOARD_HEIGHT,
                    "Ghost Y after left movement should be valid: " + ghostYLeft);
            }
            
            if (ghostYRight != -1) {
                assertTrue(ghostYRight >= 0 && ghostYRight < BOARD_HEIGHT,
    ...
    void testGhostBlockWithObstacles() {
        javafx.application.Platform.runLater(() -> {
            ...
            if (ghostY != -1) {
                assertTrue(ghostY < BOARD_HEIGHT - 1,
