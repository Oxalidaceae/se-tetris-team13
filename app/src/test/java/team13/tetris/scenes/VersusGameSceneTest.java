...
public class VersusGameSceneTest {
    ...
    void setUp() throws Exception {
        ...
        Platform.runLater(() -> {
            versusGameScene = new VersusGameScene(mockManager, mockSettings, mockEngine1, mockEngine2, false);
            latch.countDown();
        });
        latch.await();
    }
    ...
    void testTimerModeScene() throws Exception {
        CountDownLatch latch = new CountDownLatch(1);
        Platform.runLater(() -> {
            VersusGameScene timerScene = new VersusGameScene(mockManager, mockSettings, mockEngine1, mockEngine2, true);
    ...
    void testDifferentWindowSizes() throws Exception {
        ...
        for (String windowSize : windowSizes) {
            ...
            Platform.runLater(() -> {
                ...
                VersusGameScene scene = new VersusGameScene(mockManager, mockSettings, mockEngine1, mockEngine2, false);
                assertNotNull(scene);
                latch.countDown();
            });
            latch.await();
