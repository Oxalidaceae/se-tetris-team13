...
public class GameEnginePieceGenerationTest {
    ...
    void testRandomPieceGeneratesAllKinds() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (kind == Tetromino.Kind.I) hasI = true;
            if (kind == Tetromino.Kind.O) hasO = true;
            if (kind == Tetromino.Kind.T) hasT = true;
            if (kind == Tetromino.Kind.S) hasS = true;
            if (kind == Tetromino.Kind.Z) hasZ = true;
            if (kind == Tetromino.Kind.J) hasJ = true;
            if (kind == Tetromino.Kind.L) hasL = true;
        }
    ...
    void testEasyModeIPieceProbability() {
        ...
        for (int i = 0; i < totalPieces; i++) {
            ...
            if (piece.getKind() == Tetromino.Kind.I) iCount++;
        }
    ...
    void testNormalModeEqualProbability() {
        ...
        for (int i = 0; i < totalPieces; i++) {
            ...
            if (kind == Tetromino.Kind.I) counts[0]++;
            else if (kind == Tetromino.Kind.O) counts[1]++;
            else if (kind == Tetromino.Kind.T) counts[2]++;
            else if (kind == Tetromino.Kind.S) counts[3]++;
            else if (kind == Tetromino.Kind.Z) counts[4]++;
            else if (kind == Tetromino.Kind.J) counts[5]++;
            else if (kind == Tetromino.Kind.L) counts[6]++;
        }
    ...
    void testGameOverWhenPieceCannotSpawn() {
        ...
        assertNull(engine.getCurrent(), "새 조각이 생성될 수 없으면 current가 null이어야 함");
    }
