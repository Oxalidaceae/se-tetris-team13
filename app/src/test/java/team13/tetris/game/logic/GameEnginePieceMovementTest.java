...
public class GameEnginePieceMovementTest {
    ...
    void testRotateOPiece() {
        // O 블록이 나올 때까지 재시작
        while (engine.getCurrent().getKind() != Tetromino.Kind.O) engine.startNewGame();
    ...
    void testRotateFourTimes() {
        ...
        // 4번 회전 시 원래 모양으로 복귀
        assertEquals(initialShape.length, finalShape.length, "4번 회전 시 원래 크기로 복귀해야 함");
        assertEquals(initialShape[0].length, finalShape[0].length, "4번 회전 시 원래 크기로 복귀해야 함");
    }
    ...
    void testRotateWithWallKickLeft() {
        ...
        while (engine.getCurrent().getKind() != Tetromino.Kind.I) engine.startNewGame();

        // 왼쪽 벽에 붙이기
        while (engine.getPieceX() > 0) engine.moveLeft();
    ...
    void testRotateWithWallKickRight() {
        ...
        // 오른쪽 벽에 붙이기
        while (engine.getPieceX() < maxX) engine.moveRight();
    ...
    void testMoveLeftWithNullCurrent() {
        ...
        assertDoesNotThrow(() -> engine.moveLeft(), "current가 null일 때 moveLeft가 안전해야 함");
    }
    ...
    void testMoveRightWithNullCurrent() {
        ...
        assertDoesNotThrow(() -> engine.moveRight(), "current가 null일 때 moveRight가 안전해야 함");
    }
    ...
    void testRotateWithNullCurrent() {
        ...
        assertDoesNotThrow(() -> engine.rotateCW(), "current가 null일 때 rotateCW가 안전해야 함");
    }
    ...
    void testCombinedMovement() {
        ...
        assertEquals(initialX + 1, engine.getPieceX(), "연속 이동 결과가 정확해야 함");
    ...
    void testPositionPersistence() {
        ...
        assertEquals(xAfterLeft + 1, xAfterRight, "이동 후 위치가 정확히 유지되어야 함");
    }
}
