...
public class GameEngineGhostTest {
    ...
    void testGhostYOnEmptyBoard() {
        ...
        if (current != null) {
            assertTrue(board.fits(current.getShape(), engine.getPieceX(), ghostY),
                "고스트 위치에서 블록이 배치 가능해야 함");
            assertFalse(board.fits(current.getShape(), engine.getPieceX(), ghostY + 1),
    ...
    void testGhostYWithObstacles() {
        ...
        Tetromino current = engine.getCurrent();
        if (current != null) {
            assertTrue(board.fits(current.getShape(), currentX, ghostY),
                "고스트 위치에서 블록이 배치 가능해야 함");
            assertFalse(board.fits(current.getShape(), currentX, ghostY + 1),
                "고스트 위치 아래로는 배치 불가능해야 함");
        }
    }
    ...
    void testGhostYWhenAtBottom() {
        ...
        while (canDrop) {
            ...
            if (originalPiece != null && !board.fits(originalPiece.getShape(), engine.getPieceX(), engine.getPieceY() + 1)) {
            ...
            if (currentPiece == null || (originalPieceId != -1 && currentPiece.getId() != originalPieceId)) {
        ...
        assertEquals(currentY, ghostY, 
            "블록이 더 이상 떨어질 수 없을 때 고스트 Y는 현재 Y와 같아야 함 (현재Y=" + currentY + ", 고스트Y=" + ghostY + ")");
        
        // 현재 위치에서 블록이 배치 가능하고, 그 아래로는 불가능한지 확인
        Tetromino current = engine.getCurrent();
        if (current != null) {
            assertTrue(board.fits(current.getShape(), engine.getPieceX(), currentY),
                "현재 위치에서 블록이 배치 가능해야 함");
            assertFalse(board.fits(current.getShape(), engine.getPieceX(), currentY + 1),
    ...
    void testGhostYAfterHorizontalMovement() {
        ...
        Tetromino current = engine.getCurrent();
        if (current != null) {
            assertTrue(board.fits(current.getShape(), engine.getPieceX(), ghostYAfterRight),
    ...
    void testGhostYAfterHardDrop() {
        ...
        if (newCurrent != null) {
            assertTrue(board.fits(newCurrent.getShape(), engine.getPieceX(), newGhostY),
    ...
    void testGhostYForAllTetrominoTypes() {
        ...
        for (int attempt = 0; attempt < 100 && testedKinds.size() < 7; attempt++) {
            ...
            if (current != null && !testedKinds.contains(current.getKind())) {
                ...
                // then
                assertTrue(ghostY >= engine.getPieceY(), 
