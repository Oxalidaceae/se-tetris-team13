...
public class GameEngineDropTest {
  ...
  void testHardDrop() {
    ...
    assertNotSame(pieceBeforeDrop, engine.getCurrent(),
        "hardDrop 후 새 조각이 생성되어야 함");
  ...
  void testHardDropAddsScore() {
    ...
    if (dropDistance > 0) {
      assertTrue(engine.getScore() > initialScore,
          "떨어진 거리가 있으면 점수가 증가해야 함");
      assertTrue(listener.scoreChangedCount > 0,
          "점수 변경 콜백이 호출되어야 함");
    }
  }
  ...
  void testHardDropNoDistanceNoScore() {
    ...
    // 0칸 떨어졌으므로 점수 변화 없음
    assertEquals(currentScore, engine.getScore(),
        "0칸 떨어질 때는 점수가 변하지 않아야 함");
  }
  ...
  void testStartAutoDrop() throws InterruptedException {
    ...
    assertTrue(engine.getPieceY() > initialY || engine.getPieceY() == 0,
        "자동 하강으로 인해 조각이 이동했어야 함");
  }
  ...
  void testGetDropIntervalSecondsDefault() {
    ...
    assertEquals(1.0, newEngine.getDropIntervalSeconds(), 0.001,
        "기본 하강 간격은 1.0초여야 함");
  ...
  void testAddDropScore() {
    ...
    assertEquals(expectedScore, engine.getScore(),
        "소프트 드롭 점수가 정확히 계산되어야 함");
  }
  ...
  void testAddHardDropScore() {
    ...
    if (dropDistance > 0) {
      int expectedScore = initialScore +
          engine.getGameTimer().getHardDropScore(dropDistance);
      assertEquals(expectedScore, engine.getScore(),
          "하드 드롭 점수가 정확히 계산되어야 함");
    }
  }
}
