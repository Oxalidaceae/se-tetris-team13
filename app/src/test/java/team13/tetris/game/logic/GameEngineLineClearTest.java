...
public class GameEngineLineClearTest {
  ...
  void testMultipleLineClearScore() {
    ...
    assertEquals(initialScore + 1250, engine.getScore(),
        "5줄 클리어 시 1250점 추가되어야 함");
  }
  ...
  void testSixLineClearScore() {
    ...
    assertEquals(initialScore + 1500, engine.getScore(),
        "6줄 클리어 시 1500점 추가되어야 함");
  }
  ...
  void testConsecutiveLineClearScore() {
    ...
    assertEquals(initialScore + 1350, engine.getScore(),
        "연속 라인 클리어 시 점수가 누적되어야 함");
  }
  ...
  void testEasyModeSpeedIncrease() {
    GameEngine easyEngine = new GameEngine(new Board(10, 20), listener,
  ...
  void testHardModeSpeedIncrease() {
    GameEngine hardEngine = new GameEngine(new Board(10, 20), listener,
  ...
  void testDifficultySpeedIncreaseComparison() {
    GameEngine easyEngine = new GameEngine(new Board(10, 20), new TestListener(),
        ScoreBoard.ScoreEntry.Mode.EASY);
    GameEngine normalEngine = new GameEngine(new Board(10, 20), new TestListener(),
    ...
    assertTrue(hardDecrease > normalDecrease,
        "HARD 모드가 NORMAL보다 속도 증가율이 커야 함");
    assertTrue(normalDecrease > easyDecrease,
        "NORMAL 모드가 EASY보다 속도 증가율이 커야 함");
  ...
  void testTotalLinesClearedIncrease() {
    ...
    GameEngine itemEngine = new GameEngine(new Board(10, 20), listener,
    ...
    assertTrue(itemEngine.getTotalLinesCleared() >= initialTotal,
  ...
  void testGetTotalLinesCleared() {
    assertNotNull(engine.getTotalLinesCleared(),
        "getTotalLinesCleared()가 null을 반환하지 않아야 함");
    assertTrue(engine.getTotalLinesCleared() >= 0,
        "totalLinesCleared는 음수가 아니어야 함");
  }
}
