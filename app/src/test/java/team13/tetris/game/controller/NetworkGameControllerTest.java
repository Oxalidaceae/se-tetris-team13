...
class NetworkGameControllerTest {
    ...
    void testNetworkGameControllerCreationClient() {
        NetworkGameController clientController = new NetworkGameController(null, settings, false, "192.168.1.1");
    ...
    void testCreateAttackPatternNormal() throws Exception {
        ...
        Method createAttackPattern = NetworkGameController.class.getDeclaredMethod(
            "createAttackPattern", int.class, GameEngine.class);
        createAttackPattern.setAccessible(true);
        
        // GameEngine 생성
        Board board = new Board(10, 20);
        GameEngine engine = new GameEngine(board, null, team13.tetris.data.ScoreBoard.ScoreEntry.Mode.NORMAL);
    ...
    void testAddIncomingBlockToBoard() throws Exception {
        // Reflection으로 private 메서드 접근
        Method addIncomingBlockToBoard = NetworkGameController.class.getDeclaredMethod(
    ...
    void testIncomingBlocksQueue() throws Exception {
        ...
        Field myIncomingBlocksField = NetworkGameController.class.getDeclaredField("myIncomingBlocks");
    ...
    void testNetworkStabilityFields() throws Exception {
        ...
        Field lastMessageReceivedTimeField = NetworkGameController.class.getDeclaredField("lastMessageReceivedTime");
    ...
    void testUpdateLastMessageTime() throws Exception {
        // Reflection으로 private 메서드 접근
        Method updateLastMessageTime = NetworkGameController.class.getDeclaredMethod("updateLastMessageTime");
    ...
    void testTimerModeFields() throws Exception {
        Field remainingSecondsField = NetworkGameController.class.getDeclaredField("remainingSeconds");
    ...
    void testNetworkThresholdConstants() throws Exception {
        ...
        Field disconnectThresholdField = NetworkGameController.class.getDeclaredField("DISCONNECT_THRESHOLD_MS");
    ...
    void testPlayerIdFields() throws Exception {
        ...
        Field opponentPlayerIdField = NetworkGameController.class.getDeclaredField("opponentPlayerId");
    ...
    void testHostClientDistinction() {
        NetworkGameController hostController = new NetworkGameController(null, settings, true, "localhost");
        NetworkGameController clientController = new NetworkGameController(null, settings, false, "192.168.1.1");
    ...
    void testServerIPStorage() throws Exception {
        ...
        NetworkGameController testController = new NetworkGameController(null, settings, false, testIP);
    ...
    void testClientMessageListenerMethods() {
        ...
        }, "JavaFX 없는 ClientMessageListener 메서드들은 안전해야 함");
    }
    ...
    void testServerMessageListenerMethods() {
        assertDoesNotThrow(() -> {
            controller.onError("Server error");
            controller.onError(null);
        }, "onError 메서드는 JavaFX 없이 안전해야 함");
    }
    ...
    @Test
    @DisplayName("게임 시작 전 상태 테스트 - 비 JavaFX")
    void testPreGameState() throws Exception {
        // 게임 시작 상태 확인
        Field gameStartedField = NetworkGameController.class.getDeclaredField("gameStarted");
        gameStartedField.setAccessible(true);
        assertFalse(gameStartedField.getBoolean(controller), "초기 상태는 게임 시작 안 됨");
        ...
        assertNull(myEngineField.get(controller), "초기 상태에서는 엔진이 null");
    }
    ...
    void testCountdownSecondsProperty() throws Exception {
        Field countdownSecondsField = NetworkGameController.class.getDeclaredField("countdownSeconds");
    ...
    void testHostAndClientModes() {
        NetworkGameController hostController = new NetworkGameController(sceneManager, settings, true, "localhost");
        NetworkGameController clientController = new NetworkGameController(sceneManager, settings, false, "192.168.1.1");
    ...
    void testVariousServerIPAddresses() {
        ...
        for (String ip : testIPs) {
            NetworkGameController testController = new NetworkGameController(sceneManager, settings, false, ip);
    ...
    void testSendAttackPatternReflection() throws Exception {
        Method sendMethod = NetworkGameController.class.getDeclaredMethod("sendAttackPattern", int[][].class);
        sendMethod.setAccessible(true);
    ...
    void testNetworkStabilityFieldsMutation() throws Exception {
        Field lastMessageField = NetworkGameController.class.getDeclaredField("lastMessageReceivedTime");
        ...
        assertEquals(System.currentTimeMillis(), lastMessageField.getLong(controller), 1000, "lastMessageReceivedTime 설정 가능");
    ...
    void testHandleLocalGameOverReflection() throws Exception {
        Method method = NetworkGameController.class.getDeclaredMethod("handleLocalGameOver", String.class);
        method.setAccessible(true);
    ...
    void testHandleRemoteGameOverReflection() throws Exception {
        Method method = NetworkGameController.class.getDeclaredMethod("handleRemoteGameOver", String.class);
        method.setAccessible(true);
    ...
    void testVariousModeControllers() {
        ...
        for (String ip : ips) {
            for (boolean isHost : hostModes) {
                NetworkGameController testController = new NetworkGameController(
                    sceneManager, settings, isHost, ip
                );
                assertNotNull(testController, "컨트롤러 생성: " + (isHost ? "Host" : "Client") + " - IP: " + ip);
            }
        }
    }
}