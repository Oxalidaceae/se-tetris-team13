...
// Settings class 테스트: Tests user configuration management including keys,
// window size, color, and blind mode
...
public class SettingsTest {
  ...
  void testColorBlindModeGetterSetter() {
    ...
    assertTrue(settings.isColorBlindMode(),
        "Color blind mode should be true after setting");

    settings.setColorBlindMode(false);
    assertFalse(settings.isColorBlindMode(),
        "Color blind mode should be false after setting");
  }

  @Test
  @DisplayName("창 크기 기본값이 MEDIUM인지 확인")
  void testWindowSizeDefaultValue() {
    assertEquals("MEDIUM", settings.getWindowSize(),
        "Window size should be MEDIUM by default");
  }
  ...
  void testWindowSizeSmall() {
    ...
    assertEquals("SMALL", settings.getWindowSize(),
        "Window size should be SMALL after setting");
  }
  ...
  void testWindowSizeMedium() {
    ...
    assertEquals("MEDIUM", settings.getWindowSize(),
        "Window size should be MEDIUM after setting");
  }
  ...
  void testWindowSizeLarge() {
    ...
    assertEquals("LARGE", settings.getWindowSize(),
        "Window size should be LARGE after setting");
  }
  ...
  void testKeyLeftDefaultValue() {
    assertEquals("LEFT", settings.getKeyLeft(),
        "Left key should be LEFT by default");
  }
  ...
  void testKeyLeftGetterSetter() {
    ...
    assertEquals("A", settings.getKeyLeft(),
        "Left key should be A after setting");
  }
  ...
  void testKeyRightDefaultValue() {
    assertEquals("RIGHT", settings.getKeyRight(),
        "Right key should be RIGHT by default");
  }
  ...
  void testKeyRightGetterSetter() {
    ...
    assertEquals("D", settings.getKeyRight(),
        "Right key should be D after setting");
  }
  ...
  void testKeyDownDefaultValue() {
    assertEquals("DOWN", settings.getKeyDown(),
        "Down key should be DOWN by default");
  }
  ...
  void testKeyDownGetterSetter() {
    ...
    assertEquals("S", settings.getKeyDown(),
        "Down key should be S after setting");
  }

  @Test
  @DisplayName("회전 키 기본값이 Z인지 확인")
  void testKeyRotateDefaultValue() {
    assertEquals("Z", settings.getKeyRotate(),
        "Rotate key should be Z by default");
  }
  ...
  void testKeyRotateGetterSetter() {
    ...
    assertEquals("W", settings.getKeyRotate(),
        "Rotate key should be W after setting");
  }

  @Test
  @DisplayName("하드 드롭 키 기본값이 X인지 확인")
  void testKeyDropDefaultValue() {
    assertEquals("X", settings.getKeyDrop(),
        "Drop key should be X by default");
  }
  ...
  void testKeyDropGetterSetter() {
    ...
    assertEquals("SPACE", settings.getKeyDrop(),
        "Drop key should be SPACE after setting");
  }
  ...
  void testPauseDefaultValue() {
    assertEquals("P", settings.getPause(),
        "Pause key should be P by default");
  }
  ...
  void testPauseGetterSetter() {
    ...
    assertEquals("ESC", settings.getPause(),
        "Pause key should be ESC after setting");
  }
  ...
  void testIsKeyAlreadyUsedLeft() {
    assertTrue(settings.isKeyAlreadyUsed("LEFT"),
        "LEFT key should be already used");
    assertTrue(settings.isKeyAlreadyUsed("left"),
        "left key should be already used (case insensitive)");
  }

  @Test
  @DisplayName("기본 키가 이미 사용 중인지 확인 - RIGHT")
  void testIsKeyAlreadyUsedRight() {
    assertTrue(settings.isKeyAlreadyUsed("RIGHT"),
        "RIGHT key should be already used");
  }

  @Test
  @DisplayName("기본 키가 이미 사용 중인지 확인 - DOWN")
  void testIsKeyAlreadyUsedDown() {
    assertTrue(settings.isKeyAlreadyUsed("DOWN"),
        "DOWN key should be already used");
  }

  @Test
  @DisplayName("기본 키가 이미 사용 중인지 확인 - Z")
  void testIsKeyAlreadyUsedZ() {
    assertTrue(settings.isKeyAlreadyUsed("Z"),
        "Z key should be already used");
  }

  @Test
  @DisplayName("기본 키가 이미 사용 중인지 확인 - X")
  void testIsKeyAlreadyUsedX() {
    assertTrue(settings.isKeyAlreadyUsed("X"),
        "X key should be already used");
  }

  @Test
  @DisplayName("기본 키가 이미 사용 중인지 확인 - P")
  void testIsKeyAlreadyUsedP() {
    assertTrue(settings.isKeyAlreadyUsed("P"),
        "P key should be already used");
  }
  ...
  void testIsKeyAlreadyUsedUnused() {
    assertFalse(settings.isKeyAlreadyUsed("A"),
        "A key should not be already used");
    assertFalse(settings.isKeyAlreadyUsed("B"),
        "B key should not be already used");
    assertFalse(settings.isKeyAlreadyUsed("F1"),
        "F1 key should not be already used");
  }
  ...
  void testIsKeyAlreadyUsedCustomKey() {
    ...
    assertTrue(settings.isKeyAlreadyUsed("A"),
        "A key should be already used after setting as left key");
    assertFalse(settings.isKeyAlreadyUsed("LEFT"),
        "LEFT key should not be used anymore after changing");
  }
  ...
  void testIsKeyAlreadyUsedCaseInsensitive() {
    settings.setKeyLeft("a");
    assertTrue(settings.isKeyAlreadyUsed("A"),
        "Should detect 'A' as used (case insensitive)");
    assertTrue(settings.isKeyAlreadyUsed("a"),
        "Should detect 'a' as used (case insensitive)");
  }
  ...
  void testRestoreDefaultKeys() {
    ...
    // 기본값으로 복원되었는지 확인
    assertEquals("LEFT", settings.getKeyLeft(),
        "Left key should be restored to LEFT");
    assertEquals("RIGHT", settings.getKeyRight(),
        "Right key should be restored to RIGHT");
    assertEquals("DOWN", settings.getKeyDown(),
        "Down key should be restored to DOWN");
    assertEquals("Z", settings.getKeyRotate(),
        "Rotate key should be restored to Z");
    assertEquals("X", settings.getKeyDrop(),
  ...
  void testMultipleSettingsInstancesAreIndependent() {
    ...
    assertEquals("A", settings1.getKeyLeft(),
        "First instance should have A");
    assertEquals("B", settings2.getKeyLeft(),
        "Second instance should have B");
    assertNotEquals(settings1.getKeyLeft(), settings2.getKeyLeft(),
  ...
  void testWindowSizeChangeDoesNotAffectOtherSettings() {
    ...
    assertEquals("A", settings.getKeyLeft(),
        "Key settings should not change");
    assertTrue(settings.isColorBlindMode(),
        "Color blind mode should not change");
  }
