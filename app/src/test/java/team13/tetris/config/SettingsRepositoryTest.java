...
public class SettingsRepositoryTest {
  ...
  void testSaveAndLoadAllSettings() {
    ...
    // then - 모든 값이 일치하는지 확인
    assertEquals(originalSettings.isColorBlindMode(), loadedSettings.isColorBlindMode(),
        "Color blind mode should match");
    assertEquals(originalSettings.getWindowSize(), loadedSettings.getWindowSize(),
        "Window size should match");
    assertEquals(originalSettings.getKeyLeft(), loadedSettings.getKeyLeft(),
        "Left key should match");
    assertEquals(originalSettings.getKeyRight(), loadedSettings.getKeyRight(),
        "Right key should match");
    assertEquals(originalSettings.getKeyDown(), loadedSettings.getKeyDown(),
        "Down key should match");
    assertEquals(originalSettings.getKeyRotate(), loadedSettings.getKeyRotate(),
        "Rotate key should match");
    assertEquals(originalSettings.getKeyDrop(), loadedSettings.getKeyDrop(),
  ...
  void testMultipleConcurrentSaves() {
    for (int i = 0; i < 10; i++) {
      ...
      assertDoesNotThrow(() -> SettingsRepository.save(settings),
          "Should handle multiple saves");
    }
