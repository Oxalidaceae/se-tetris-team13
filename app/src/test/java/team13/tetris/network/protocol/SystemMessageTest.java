...
class SystemMessageTest {
    ...
    void testConstructor() {
        SystemMessage message = new SystemMessage("Client", "Test message", SystemMessage.SystemLevel.WARNING);
    ...
    void testNullLevel() {
        ...
        assertNotNull(message, "메시지가 생성되어야 함");
        assertEquals(SystemMessage.SystemLevel.ERROR, message.getLevel(), "null 레벨일 때 기본값 ERROR를 사용해야 함");
    }
    ...
    void testVariousMessages() {
        ...
        // 긴 메시지
        String longMessage = "This is a very long error message that contains a lot of detailed information about what went wrong in the system.";
        SystemMessage long1 = SystemMessage.createError("Server", longMessage);
        assertEquals(longMessage, long1.getMessage());
        
        // 특수 문자 포함 메시지
        SystemMessage special = SystemMessage.createWarning("Server", "Error at line 42: NullPointerException");
    ...
    void testAllLevels() {
        ...
        SystemMessage warning = new SystemMessage("S", "Warning", SystemMessage.SystemLevel.WARNING);
