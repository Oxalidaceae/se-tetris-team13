    ...
    void testGhostBlockAfterGameStart() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertDoesNotThrow(() -> gameScene.updateGrid(), "updateGrid가 예외 없이 실행되어야 함");

                    // then - 고스트 위치가 유효해야 함
                    assertTrue(ghostY >= 0, "고스트 Y가 유효해야 함: " + ghostY);
                    assertTrue(ghostY < 20, "고스트 Y가 보드 범위 내에 있어야 함: " + ghostY);
                    assertTrue(ghostY >= gameEngine.getPieceY(), "고스트 Y가 현재 블록 Y 이상이어야 함");
    ...
    void testGhostBlockUpdatesWithMovement() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    assertDoesNotThrow(() -> gameScene.updateGrid(), "이동 후 렌더링이 예외 없이 실행되어야 함");
                });
        ...
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    if (ghostYAfterRotation != -1) {
                        assertTrue(
                                ghostYAfterRotation >= 0 && ghostYAfterRotation < 20,
                    ...
                    // 렌더링이 정상 동작해야 함
                    assertDoesNotThrow(() -> gameScene.updateGrid(), "회전 후 렌더링이 예외 없이 실행되어야 함");
                });
    ...
    void testGhostBlockInColorBlindMode() {
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    // when - 고스트 블록 렌더링
                    assertDoesNotThrow(
                            () -> gameScene.updateGrid(), "색맹 모드에서 고스트 블록 렌더링이 예외 없이 실행되어야 함");

                    // then - 고스트 위치가 여전히 유효해야 함
                    int ghostY = gameEngine.getGhostY();
                    if (ghostY != -1) {
                        assertTrue(ghostY >= 0 && ghostY < 20, "색맹 모드에서도 고스트 Y가 유효해야 함: " + ghostY);
                    }
        ...
        javafx.application.Platform.runLater(
                () -> {
                    ...
                    if (newPiece != null
                        ...
                        assertDoesNotThrow(
                                () -> gameScene.updateGrid(), "하드 드롭 후 렌더링이 예외 없이 실행되어야 함");
                    }
                });
